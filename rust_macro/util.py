...
def splitargs(tokens: Iterable[Token], delimiter: str =',') -> List[List[Token]]:  # typehinting is a bit broken
    """
    Splits a token stream into its args, separated by a delimiter
    """
    return [list(y) for x, y in itertools.groupby(tokens, lambda z: z.string == delimiter) if not x]
